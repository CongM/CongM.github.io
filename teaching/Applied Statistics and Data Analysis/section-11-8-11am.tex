...
\date{November 8, 2019} % Date, can be changed to a custom date
...
\subsubsection{Factors and Quantitative Predictors}


\begin{frame}
	
	\frametitle{Factors and Quantitative Predictors}
	
	\begin{itemize}
		\item \textbf{Factor Predictors}  (\texttt{factor} in R):
		 A two-level factor variable could be represented by one dummy variable $ c $ where
		\begin{equation}
			c =
			\begin{cases}
			...
			\end{cases}
		\end{equation}
		\item \textbf{Regression with Factors and Quantitative Predictors}
		$$ y = \beta_0 + \beta_1 x + \beta_2 c + \beta_3 x c. $$
		\begin{itemize}
			\item Reference Level ($ c = 0 $)
			$$ y = \beta_0 + \beta_1 x. $$
			\item Treatment Level ($ c = 1 $)
...
\subsubsection{Interpretation with Interaction Terms}


\begin{frame}
	
	\frametitle{Interpretation with Interaction Terms}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			$$ y = (\beta_0 + \beta_2 )+ (\beta_1 + \beta_3) x . $$
		\end{itemize}
	    \item Center $ x $ by replacing it with $ x - \overline{x} $
	\end{itemize}
...
\subsubsection{Factors With More Than Two Levels}
...
\begin{frame}
	...
	\begin{itemize}
		\item In general, a $ d $-level factor variable could be represented by $ d - 1 $ dummy variables $ c_2, \cdots, c_d $ where (assuming level 1 is the reference level)
		\begin{equation}
		c_i =
		\begin{cases}
		0 & \text{not level} \; i \\
		...
		\end{cases}
		\end{equation}
		\item \textbf{Sequential Analysis of Variance} (\texttt{anova} in R)
...
\begin{frame}
	...
	\frametitle{Examples in Faraway Chapter 14}
	
	
	\begin{itemize}
		\item \textbf{Example:} \texttt{sexab} dataset
		\item \textbf{Example:} \texttt{whiteside} dataset
		\item \textbf{Example:} \texttt{fruitfly} dataset
	\end{itemize}
