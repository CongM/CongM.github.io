...
\date{September 13, 2019} % Date, can be changed to a custom date
...
\begin{frame}
	...
	\frametitle{Least Squares Formulation}
	...
	$$ L = \sum_{i=1}^{n} \epsilon_i^2 = \epsilon^\top \epsilon = \left(y - X \beta \right)^\top \left(y - X \beta \right). $$
	$$ \frac{\partial L}{\partial \beta} = 0 \qquad \Longrightarrow \qquad \left(X^\top X \right) \beta = X^\top y. $$
	$$ \widehat{\beta} = \left(X^\top X \right)^{-1} X^\top y, \qquad \widehat{y} = X \widehat{\beta} = X \left(X^\top X \right)^{-1} X^\top y. $$
...
\begin{frame}
	...
	\begin{itemize}
		\item $ y = \mu + \epsilon $
		$$ X = \mathbf{1}, \beta = \mu \quad \Longrightarrow \quad \widehat{\beta} = \left(\mathbf{1}^\top \mathbf{1} \right)^{-1} \mathbf{1}^\top y = \frac{1}{n} \mathbf{1}^\top y = \bar{y}. $$
		\item $ y = \beta_0 + \beta_1 x + \epsilon \quad \Longleftrightarrow \quad y = \beta_0 + \beta_1 \bar{x} + \beta_1 (x - \bar{x}) + \epsilon $
		\[
		X = 
		\begin{bmatrix}
		1 & x_{1} - \bar{x} \\
		...
		\begin{bmatrix}
		n & 0 \\
		0 & \sum_{i=1}^{n} \left(x_i - \bar{x} \right)^2
...
\begin{frame}
	...
	\begin{block}{Gauss-Markov Theorem}
		...
		$$ \mathbb{E} \left[\widehat{\beta}_{OLS} \right] = \beta. $$
		$$  Var \left[\widehat{\beta}_{OLS} \right] \leq Var \left[\widehat{\beta} \right] \qquad \text{for} \qquad \widehat{\beta} \in \left\{\widehat{\beta} \; | \; \mathbb{E} \left[\widehat{\beta} \right] = \beta \right\}. $$
	\end{block}
...
\begin{frame}
	...
	\begin{itemize}
		\item \textbf{Sum of Squares Total}
		$$ SSTO = \sum_{i=1}^{n} \left(y_i - \bar{y} \right)^2. $$
		\item \textbf{Sum of Squares Regression}
...
\subsection{Example with \texttt{gala} Dataset}


\begin{frame}
	
	\frametitle{Data - \texttt{gala}}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \texttt{Species}: the number of species found on the island
			\item \texttt{Area}: the area of the island (km2)
			\item \texttt{Elevation}: the highest elevation of the island (m)
			\item \texttt{Nearest}: the distance from the nearest island (km)
			\item \texttt{Scruz}: the distance from Santa Cruz Island (km)
			\item \texttt{Adjacent}: the area of the adjacent island (km2)
		\end{itemize}
	\end{itemize}
...
\subsection{Exercise with \texttt{teengamb} Dataset}
