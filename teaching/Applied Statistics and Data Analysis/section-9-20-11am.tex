...
\date{September 20, 2019} % Date, can be changed to a custom date
...
\subsection{Linear Combinations of Two Normals}
...
\begin{frame}
	...
	\frametitle{Linear Combinations of Two Normals}
	
	\begin{block}{Claim}
		Let $ U = aX_1, V = bX_2 $ where $ X_1 \sim \mathcal{N}(\mu_1, \sigma_1^2) $ and $ X_2 \sim \mathcal{N}(\mu_2, \sigma_2^2) $. Then $ U \sim \mathcal{N}(a\mu_1, a^2\sigma_1^2) $ and $ V \sim \mathcal{N}(b\mu_2, b^2\sigma_2^2) $.
	\end{block}
	
	\begin{block}{Question}
		Assume $ X_1 $ and $ X_2 $ are independent. What can you say about $ Y = U + V $ ?
	\end{block}

	\begin{block}{Question}
		How to show a random variable follows some distribution ?
	\end{block}
...
\begin{frame}
	...
	\frametitle{Exercises in Faraway Chapter 2}
	
	
	\begin{itemize}
		\item \textbf{Exercise 2:} \texttt{uswages} dataset
		\item \textbf{Exercise 3:} artificial data
		\item \textbf{Exercise 4:} \texttt{prostate} dataset
	\end{itemize}
